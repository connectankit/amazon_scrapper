...
async function scrapeAmazonRefrigerators(baseURL, startPage=1, endPage) {
    const data = [];
    ...
    for (let page = startPage; page <= endPage; page++) {
        ...
        try {
            ...
            $('div[data-component-type="s-search-result"]').each((index, element) => {
                ...
                const textElements = $(element).find('span.a-text-normal');
                const texts = textElements.map((index, el) => $(el).text().trim()).get();
                sectionData.texts = texts.length ? texts : null;

                const imageElement = $(element).find('img.s-image-optimized-rendering');
                const imageUrl = imageElement.attr('src');
                sectionData.image_url = imageUrl || null;

                const priceElement = $(element).find('span.a-price-whole');
                sectionData.price = priceElement.text().trim() || null;

                const marketPriceElement = $(element).find('span.a-offscreen').first();
                sectionData.market_price_rate = marketPriceElement.text().trim() || null;

                const bankOfferElement = $(element).find('span.a-truncate-cut').first();
                sectionData.bank_offer = bankOfferElement.text().trim() || null;

                const freeDeliveryElement = $(element).find('span.a-color-base').first();
                sectionData.free_delivery = freeDeliveryElement.text().trim() || null;

                const ratingElement = $(element).find('span.a-icon-alt').first();
                sectionData.rating_text = ratingElement.text().trim() || null;

                if (sectionData.image_url !== null && sectionData.price !== null) {
